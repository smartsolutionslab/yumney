...
internal static class ShoppingListTestData
{
    ...
    public static ShoppingList CreateListWithItems(string ownerId = "user-123")
    {
        return ShoppingList.Create(
            ...
            OwnerIdentifier.From(ownerId),
            [
                ShoppingListItem.Create(ItemName.From("Milk"), Quantity.Of(Amount.From(1), Unit.Liters)),
                ShoppingListItem.Create(ItemName.From("Flour"), Quantity.Of(Amount.From(500), Unit.Grams)),
    ...
    {
        var item = ShoppingListItem.Create(ItemName.From("Milk"), Quantity.Of(Amount.From(1), Unit.Liters));
