...
/// implementations. Each invocation routes through <see cref="IInboxStore.ProcessAsync"/>
/// so the inbox row and the handler's writes share a single transaction with retry-aware
/// semantics: the row only commits on handler success, a thrown handler rolls the row back
/// so a redelivery can retry the work, and a duplicate-row race is treated as already-processed.
/// </summary>
	...
	where TEvent : class, IIntegrationEvent
{
		...
		CancellationToken cancellationToken)
	{
		...
		try
		{
			ranHandler = await inboxStore.ProcessAsync(
