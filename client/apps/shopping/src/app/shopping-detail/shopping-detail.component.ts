...
export class ShoppingDetailComponent implements OnInit {
  ...
  protected readonly availableCategories = CATEGORY_KEYS;
  ...
  private optimisticBatchUpdate(checked: boolean): void {
    ...
    const previousStates = list.items.map((item) => item.isChecked);
    optimisticSignalUpdate(
      this.shoppingList,
      this.destroyRef,
      () => {
      ...
      () => {
        list.items.forEach((item, index) => (item.isChecked = previousStates[index]));
      },
      (current) => this.shoppingApi.checkOffAllItems(current.identifier, checked),
    );
  }
}
